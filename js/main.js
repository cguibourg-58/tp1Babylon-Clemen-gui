...
function startGame() {
    ...
    //var spheres = scene.getMeshByName("mySphere");
    let tankCanon = scene.getMeshByName("heroTankCanon");
    ...
    engine.runRenderLoop(() => {
        ...
        if(heroDude) {
            ...
            //heroDude.move(); ne fonctionne pas pour une raison inconnue
        }
...
function createScene() {
    /*var spheres = BABYLON.MeshBuilder.CreateSphere("mySphere", {diameter: 2, segments: 32}, scene);
    spheres.position.x = 3;
    spheres.position.y = 2;

    sphereMaterials = new BABYLON.StandardMaterial("sphereMaterial", scene);
    spheres.material = sphereMaterials;

    mirrorMaterial.reflectionTexture.renderList.push(spheres);*/
...
function createGround(scene) {
    const groundOptions = { width:2000, height:2000, subdivisions:20, minHeight:0, maxHeight:300, onReady: onGroundCreated};
    //scene is optional and defaults to the current scene
    //const ground = BABYLON.MeshBuilder.CreateGroundFromHeightMap("gdhm", 'images/hmap1.png', groundOptions, scene); 
    const ground = BABYLON.MeshBuilder.CreateGroundFromHeightMap("gdhm", 'images/hmap3.jpg', groundOptions, scene); 
...
function createTank(scene) {
    ...
    tankMaterial.diffuseTexture = new BABYLON.Texture("images/tanktexture.png");
...
function createBall(scene) {
    let ball = BABYLON.MeshBuilder.CreateSphere("heroTankCanon", {diameter: 4, segments: 32}, scene);
    let ballMaterial = new BABYLON.StandardMaterial("tankCanonMaterial", scene);
    //tankMaterial.diffuseColor = new BABYLON.Color3.Red;
    //tankMaterial.emissiveColor = new BABYLON.Color3.Blue;
    ...
    ball.position.z = 5;
    ball.speed = 1;
    ball.frontVector = new BABYLON.Vector3(0, 0, 0);
    ballMaterial.diffuseTexture = new BABYLON.Texture("images/tanktexture.png");
    var colorBall=new BABYLON.Color3.Red;
    ballMaterial.emissiveColor = colorBall;

    let counter = 0;
    ball.move = () => {
        ball.position.y = 8+2*Math.sin((counter)); 
        counter += 0.1;
...
function createHeroDude(scene) {
   // load the Dude 3D animated model
    ...
    BABYLON.SceneLoader.ImportMesh("him", "models/Dude/", "Dude.babylon", scene,  (newMeshes, particleSystems, skeletons) => {
        ...
        var a = scene.beginAnimation(skeletons[0], 0, 100, true, heroMouv);
        let hero = new Dude(heroDude, 0.5);
